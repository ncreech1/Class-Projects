...
/* Nicholas Creech
 * 9/7/19
 * Description: Wrtie a program that takes a pgm file and outputs the same pgm file but flipped vertically
 */
...
int main()
{
	...
	{
		//At the end of each row (starting from the end of the vector), print the row
		if((x + 1) % columns == 0)
		{
			for(int i = x - (columns - 1); i <= x; i++)
			{
				printf("%d\n", pixels[i]);
			}
		}
